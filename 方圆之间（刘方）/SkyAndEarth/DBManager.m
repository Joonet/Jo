...
-(void)insertModel:(TravelModel *)model{
//    FeaturedModel *Model = (FeaturedModel *)model;
//    NSLog(@"%@",model.Mid);
    [_database open];
    if ([self isExistAppForMid:model.travelId]) {
//        NSLog(@"this app has  recorded");
        return;
    }
    NSString *sql = @"insert into appInfo(name,id,front_cover_photo_url,start_date,days) values (?,?,?,?,?)";
    BOOL isSuccess = [_database executeUpdate:sql,model.name,model.travelId,model.front_cover_photo_url,model.start_date,model.days];
    if (!isSuccess) {
...
- (void)deleteAll{
    [_database open];
    NSString *sql = @"drop table  appInfo";
...
- (NSArray *)readModels{
    ...
    while ([rs next]) {
        ...
        Model.travelId = [rs stringForColumn:@"id"];
